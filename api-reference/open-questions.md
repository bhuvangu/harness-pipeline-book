...
and corpus limitations found while writing this guide. Each entry says what
we observed, why it matters, and what to ask a colleague (or verify against
a live account).

## Corpus vs. prompt path count
...
## Execution status: where is `Success`?
...
## Keyword over-inclusion in the schema digest and path tree
...
## GitX docs absent from the corpus
...
## Identifier regex inconsistencies (spec-internal)
...
## Variable types: String-only API vs richer docs
...
## v1-beta API coverage is uneven
...
## Feature flags observed (beta surface area)
...
## Trigger v2 endpoints, undocumented split
...
## Environment "type" vocabulary mismatch (minor)
...
## Where is stage-level CRUD for CD steps' rollback state?
...
## Aggregate org/project APIs not in corpus
...
impact, but the Account/Organization/Project entries in the entity reference cite
structure rather than CRUD docs.
