---
sidebar_position: 22
title: Unified CI/CD GAR GKE pipeline
...
Next, click **+ ADD PRINCIPAL** from the **Permissions** tab and type in `allUsers` for the principal and `Artifact Registry Reader` for the role. You might see a warning like this: `This resource is public and can be accessed by anyone on the internet.`
...
For the **imageTag**, click the thumbtack icon and select **Expression**. Every time you run the pipeline, the pipeline sequence ID will change, and subsequently, the image that will be built and deployed will also change.
